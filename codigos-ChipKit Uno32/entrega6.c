...
//Funcion que controlara un ServoMotor utilizando los valores analogicos de la lectura de un Potenciometro
void Servo_Control_PWM(int x, int DC) {
    int DutyCA = DC * PWM_PERIOD / 1023;
    int Servo = 0.05 * DutyCA + 1249;
    if (x == 1)
        OC1RS = Servo;
    else
        OC2RS = Servo;
...
main() {
    ...
    OpenOC2(OC_ON | OC_TIMER3_SRC | OC_PWM_FAULT_PIN_DISABLE, 0, 0); // Configuracion de OutputCompare2 para generar salidas PWM
...
void __ISR(_TIMER_1_VECTOR, ipl2) Timer1Handler(void) {
    ...
    if (contLed0 == 700)
        ...
        if (PORTEbits.RE0 && contLed0 <= 500) {
            PORTDbits.RD5 = 1;
        } else if (PORTEbits.RE0 && contLed0 > 500 && contLed0 <= 700) {
            PORTDbits.RD5 = 0;
        } else {
            PORTDbits.RD5 = 0;
            contLed0 = 0;
    ...
    if (contLed1 == 700)
        ...
        if (PORTEbits.RE1 && contLed1 <= 500) {
            PORTDbits.RD11 = 1;
        } else if (PORTEbits.RE1 && contLed1 > 500 && contLed1 <= 700) {
            PORTDbits.RD11 = 0;
        } else {
            PORTDbits.RD11 = 0;
            contLed1 = 0;
